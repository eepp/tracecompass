...
public abstract class LamiAnalysis implements IOnDemandAnalysis {
    ...
    /**
     * Map of pre-defined views, for every table class names.
     *
     * If a table class is not in this map then it means that table has not
     * predefined views.
     *
    ...
    private boolean checkMetadata() {
        ...
        try {
            ...
            for (String tableClassName : tableClassNames) {
                ...
                Collection<LamiChartModel> chartModels = getPredefinedViews().get(tableClassName);
            ...
            try {
                ...
                /*
                 * This is thrown if there are duplicate keys in the map builder.
                 */
    ...
    private static List<LamiTableEntryAspect> getAspectsFromColumnDescriptions(JSONArray columnDescriptions) throws JSONException {
        ...
        for (int j = 0; j < columnDescriptions.length(); j++) {
            ...
            case TIME_RANGE:
                /*
                 * We will add 3 aspects, to represent the start, end
                 * and duration of this time range.
                 */
            ...
            default:
                ...
                LamiTableEntryAspect aspect = new LamiGenericAspect(columnTitle,
                        units, colIndex, columnDataType.isNumerical(), false);
                aspectsBuilder.add(aspect);
                break;
            }
        }
        /*
         * SWT quirk : we need an empty column at the end or else the
         * last data column will clamp to the right edge of the view if
         * it is right-aligned.
         */
     ...
     *
     *
     * @return Should not be null after the execution completed successfully.
     */
     ...
     * @return Should not be null after the execution completed successfully.
     */
    public @Nullable Map<String, LamiTableClass> getTableModels() {
     ...
     */
    public String getBaseCommandAsString(ITmfTrace trace, @Nullable TmfTimeRange range) {
        ImmutableList.Builder<String> builder = getBaseCommand(range);
        builder.add(checkNotNull(trace.getPath()));
    ...
    /**
     * Call the previously defined Babeltrace script with the given arguments.
     *
     * @param timeRange
    ...
    public List<LamiResultTable> execute(ITmfTrace trace, @Nullable TmfTimeRange timeRange,
        ...
        try {
            ...
            if (results.length() == 0) {
                /*
                 * No results were reported. This may be normal, but warn the user
                 * why a report won't be created.
                 */
                throw new OnDemandAnalysisException(Messages.LamiAnalysis_NoResults);
            }
            ...
            for (int i = 0; i < results.length(); i++) {
                ...
                LamiTimeRange tr = getTimeRangeFromObject(trObject);
        ...
        return checkNotNull(resultsBuilder.build());
    }

    /**
     * @param obj
     */
    private static LamiTimeRange getTimeRangeFromObject(JSONObject obj) {
        // FIXME NYI
        return new LamiTimeRange(0, 0);
    }
...
//                    System.out.println(line);
                    ...
                    // Trim the line first to make sure the first character is significant
                    line = line.trim();
                    ...
                    if (elems[0].matches("\\d.*")) { //$NON-NLS-1$
                        ...
                        try {
                            ...
                            // We're going backwards? Do not update the monitor's value
                            if (workedThisLoop > 0) {
